...
tbody.innerHTML = '';
    ...
    console.error('❌ Error cargando distritos:', error);
    const tbody = document.querySelector('#tablaDistritos tbody');
    if (tbody) tbody.innerHT ML = ' <tr > <td colspan= "4 " >Error cargando datos </td > </tr >';
}
}
...
tbody.innerHTML = '';
    ...
    console.error('❌ Error cargando conferencias:', error);
    const tbody = document.querySelector('#tablaConferencias tbody');
    if (tbody) tbody.i nnerHTML = ' <tr > <td colspan= "7 " >Error cargando datos </td > </tr >';
}
}
...
tbody.innerHTML = '';
    ...
    const tbody = document.querySelector('#tablaAsistentes tbody');
    if (tbody) tbody.inner HTML = ' <tr > <td colspan= "6 " >Error: ' + error.message + ' </td > </tr >';
}
}
...
const fechas_asistencia = obtenerFechasSeleccionadas();
    ...
    cerrarModal('modalNuevoAsistente');
    await cargarAsistentes} catch (error) {
...
if (!usuario) return;
    ...
    document.getElementById('us uarioEmail').value = usuario.email;
    document.getElementById('usuarioPassword').value = '';
    document.getElementById('usuarioRol').value = usuario.rol;
    document.getElemen tById('usuarioEstado').value = usuario.estado || 'activo';
    ...
    document.getElementById('tituloModalUsua rio').textContent = '👤 Editar Usuario';
...
const iglesiaId = document.getElementById('reporteIglesia')?.value;
    ...
    if (asistentes  & & asistentes.length  > 0) {
        ...
        const elTotalAsis tentes = document.getElementById('reporteTotalAsistentes');
        const elTotalDias = document.getElementById('reporteTotalDias');
        const elTotalIglesias = document.getEle mentById('reporteTotalIglesias');
        
        if (elTotalAsistentes) elTotalAsistentes.textContent = asistentes.length;
        if (elTotalDias) elTotalDias.textContent = tota lDias;
        ...
        const elTotalDias = document.getElementById('reporteTotalDias');
        const elTo talIglesias = document.getElementById('reporteTotalIglesias');
    ...
    console.error('❌ Error filtrando report:', error);
...
const doc = new jsPDF();
    ...
    let asistentes = aw ait obtenerAsistentes();
    ...
    const conferencias = await obtenerConferencias();
    const iglesias = await obtenerIglesias();
    const conferenciaFiltro = conferenc iaId ? conferencias.find(c = > c.id == conferenciaId)?.nombre : 'Todas';
    ...
    doc.text(`Conferencia: ${conferen ciaFiltro}`, 14, 58);
    ...
    doc.autoTable({
        startY: 105,
        head: [['Nombre', 'Teléfono', 'Iglesia', 'Conferencia', 'Días', 'Invitado P or']],
        body: tableData,
        theme: 'striped',
    ...
    const finalY = doc.la stAutoTable.finalY || 105;
    doc.setFontSize(9);
    ...
    mostrarMensaje('✅ Repo rte PDF generado exitosamente 'success');
} catch (error) {
    console.error('❌ Error generando PDF:', error);
    mostrarMensaje('❌ Error al generar PDF: ' + error.message, 'er ror');
}
}
// ============================================
// INICIALIZACIÓN
...
e.preventDefault();
        ...
        const btnLogin = loginForm.querySelector('.btn-log in');
        ...
        try {
            ...
            if (resultado .success) {
                ...
                setTimeout(() = > {
                    ...
                    if (confInicio  & & confFin) {
                        confInicio.addEventListener('change', actualizarDuracionConferencia);
                        confFin.addEventListener('change', actualizarDura cionConferencia);
                    }
...
window.prepararModalConferencia = prepara rModalConferencia;
...
window.confirmarEliminarIglesia = confirmarEliminarIglesi a;
...
window.cargarAsistent es = cargarAsistentes;
window.guardarAsistente = guardarAsistente;
window.editarUsuario = editarUsuario;
window.guardarUsuarioEditado = guardarUsuarioEditado;
window.confirmarElimi narUsuario = confirmarEliminarUsuario;
...
w indow.actualizarContadorAsistencia = actualizarContadorAsistencia;
window.obtenerFechasSeleccionadas = obtenerFechasSeleccionadas;
window.marcarFechasGuardadas = marcarFechasGuarda das;
window.actualizarDuracionConferencia = actualizarDuracionConferencia;
window.abrirModal = abrirModal;
window.cerrarModal = cerrarModal;
window.cargarEstadisticas = cargarEstad isticas;
...
window.ca rgarReportes = cargarReportes;
...
window.mo strarMensaje = mostrarMensaje;
